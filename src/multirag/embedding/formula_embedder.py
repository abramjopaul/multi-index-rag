...
from multirag.formula_search import (FormulaTokenizerPipeline,
...
class FormulaEmbedder:
    ...
    def __init__(
        ...
        self.model_path = model_path or str(
            get_default_training_dir() / "fasttext_model.bin"
        )
    ...
    def embed_batch(
        ...
        with tqdm(total=len(formulas_df), desc="Embedding", unit="formula") as pbar:
            for idx, row in formulas_df.iterrows():
                formula_id = (
                    row.get(formula_id_column, idx) if formula_id_column else idx
                )
