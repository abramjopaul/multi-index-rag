...
class DataScopeConfig(BaseModel):
    ...
    max_formulas: Optional[int] = Field(
        default=None, ge=1, description="Max formulas to process (None = all)"
    )
    ...
    skip_first_n: int = Field(
        default=0, ge=0, description="Number of formulas to skip from beginning"
    )
...
class RepresentationConfig(BaseModel):
    ...
    tokenization_mode: Literal["value", "type", "both_separated", "both_combined"] = (
        Field(default="both_separated", description="How to encode tuple elements")
    )
...
class PathsConfig(BaseModel):
    ...
    index_path: str = Field(
        default="data/indices/formula/", description="FAISS index storage directory"
    )
