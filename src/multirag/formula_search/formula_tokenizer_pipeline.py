...
class FormulaTokenizerPipeline:
    ...
    def __init__(
        ...
        if self.encoder_maps_path and Path(self.encoder_maps_path).exists():
            try:
                ...
                logger.warning(f"Failed to load encoder maps from {self.encoder_maps_path}: {e}")
                self.token_manager = TokenIDManager()
        else:
            self.token_manager = TokenIDManager()
    ...
    def tokenize_formula(
        ...
        try:
            ...
            if tree is None:
                logger.warning(f"Failed to generate {tree_type} for formula: {latex_formula}")
                return None
            
            # Step 2: Extract tuples from tree
            tuples = tree.get_pairs(window=window, eob=include_eob)
            if not tuples:
                logger.warning(f"No tuples extracted from {tree_type} for formula: {latex_formula}")
                return None
